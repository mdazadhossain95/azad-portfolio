...
export async function generateMetadata(
  ...
  return {
    title: `${article.meta.title} | V4 Article | Azad Portfolio`,
    description: article.meta.preview,
    alternates: {
      canonical: `https://azadhossain.dev/v4/articles/${slug}`,
    },
  };
}

export default async function V4ArticleDetailPage(props: ArticleDetailPageProps) {
  const { slug } = await props.params;
  const article = await getArticleBySlug(slug);
  ...
  return (
    <section className="mx-auto w-full max-w-3xl px-5 py-20 md:px-8 md:py-24">
      <nav className="mb-8 flex items-center gap-2 text-sm text-[var(--text-muted)]">
        <Link href="/v4" className="text-[var(--accent)] hover:underline">
          Home
        </Link>
        <span aria-hidden="true">/</span>
        <Link href="/v4/articles" className="text-[var(--accent)] hover:underline">
          Articles
        </Link>
      </nav>
